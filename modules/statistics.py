...
def get_stats(db, ch, pos):
	...
	map_i = cursor.execute('''SELECT mappability_score FROM db2.GlenX as a where a.chrom=? and a.start_pos = ? ''', (ch, pos1)).fetchone()[0]
	gc_content = cursor.execute('''SELECT GC_content FROM db2.GlenX as a where a.chrom=? and a.start_pos between ? and ?''', (ch, pos1, pos2)).fetchone()[0]
	cursor.execute('''SELECT avg(read_coverage) FROM read_cov where chrom=? and start_pos between ? and ?''', (ch, pos1, pos2))
	r_i = cursor.fetchone()[0]
	if r_i is None:
		return statistics
	else:	
